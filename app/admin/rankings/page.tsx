...
import { BONUS_AWARDS, STUDENTS } from "@/lib/mock-data";
...
export default function RankingsPage() {
  ...
  const leaderboard = useMemo(() => computeLeaderboard(awards), [awards]);
  const history = useMemo(
    () =>
      [...BONUS_AWARDS, ...awards].sort(
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Combined standing across assignments, quizzes, and bonus points.
          </p>
        </div>
            ...
            {leaderboard.map((row, i) => (
              <tr
                ...
                className={`border-b border-slate-50 last:border-0 ${
                  i < 3 ? "bg-brand-tint/40" : ""
                }`}
              >
                <td className="px-5 py-4 tabular-nums text-slate-500">{i + 1}</td>
                <td className="px-5 py-4">
                  <span
                    className={`text-foreground ${i < 3 ? "font-semibold" : "font-medium"}`}
                  >
                    {row.studentName}
                  </span>
                </td>
                ...
                <td className="px-5 py-4 tabular-nums font-semibold text-foreground">
                  {row.total ?? 0}
                </td>
              </tr>
            ))}
          ...
          {leaderboard.map((row, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-sm text-foreground ${i < 3 ? "font-semibold" : "font-medium"}`}>
                    {row.studentName}
                  </p>
      ...
      <Card className="mt-6">
        <div className="border-b border-slate-100 px-5 py-4">
          <h2 className="text-sm font-semibold text-foreground">
            Performance point history
          </h2>
        </div>

        {history.length === 0 ? (
          <p className="px-5 py-8 text-center text-sm text-slate-400">
            No performance points awarded yet.
          </p>
        ) : (
          <ul>
            {history.map((award) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  {award.reason && (
                    <p className="mt-0.5 text-xs text-slate-500">{award.reason}</p>
                  )}
                </div>
        ...
        <form onSubmit={handleAward} className="space-y-4">
          ...
          <div>
            ...
            <Textarea
              ...
              rows={2}
              placeholder="e.g., Outstanding participation in lab session"
            />
          </div>
